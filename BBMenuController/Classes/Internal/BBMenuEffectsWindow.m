...
@interface BBMenuEffectsWindow ()
@property (nonatomic, strong) UITapGestureRecognizer * tapGesture;
@property (nonatomic, weak) BBMenuViewContainer * currentMenu;
@end

@implementation BBMenuEffectsWindow

+ (instancetype)sharedWindow
{
    static BBMenuEffectsWindow *inst = nil;
    if (inst == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
            inst = [self new];
            inst.frame = (CGRect){.size = BBMenuScreenSize()};
...
- (void)showMenu:(BBMenuViewContainer *)menu animation:(BOOL)animation
{
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut  animations:^{
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:BBMenuControllerDidShowMenuNotification object:nil];
