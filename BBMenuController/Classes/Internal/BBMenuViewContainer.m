...
#define BBMenuStatusBarHeight  [UIApplication sharedApplication].statusBarFrame.size.height
#define BBMenuScreenHeight [UIScreen mainScreen].bounds.size.height
#define BBMenuScreenWidth [UIScreen mainScreen].bounds.size.width
static inline CGPoint BBMenuGetXCenter(CGRect rect) {
    return CGPointMake(CGRectGetMidX(rect), rect.origin.y);
}


@interface BBMenuViewContainer()
{
    CGFloat _arrowMidx;
}
...
@implementation BBMenuViewContainer
...
- (void)dealloc
{
    //NSLog(@"BBMenuController Dealloc~");
}
...
- (void)layoutSubviews
{
    ...
    CGRect roundedRect = CGRectMake(0, (self.CorrectDirection == BBMenuControllerArrowUp ? 0 : arrowSize.height), CGRectGetWidth(self.bounds) , CGRectGetHeight(self.bounds) - arrowSize.height);
    CGRect menuRect = self.menuView.frame;
    menuRect.origin.y = (self.CorrectDirection == BBMenuControllerArrowUp ? 0 : arrowSize.height);
    ...
    if (self.CorrectDirection == BBMenuControllerArrowUp) {
        //始画三角形，箭头向下
    ...
    if (self.CorrectDirection == BBMenuControllerArrowDown) {
        // 箭头向上
...
- (void)setImagePosition:(BBAdjustButtonIMGPosition)imagePosition
{
...
- (void)processMenuFrame
{
    ...
    CGRect rect = [self.targetView convertRect:self.targetRect toView:[BBMenuEffectsWindow sharedWindow]];
   
    if (self.menuView.superview) {
        [self.menuView removeFromSuperview];
    }
    self.menuView = [BBMenuDefaultView defaultView:self WithMenuItems:_menuItems MaxSize:CGSizeMake(_maxMenuViewWidth-_menuEdgeInset.left-_menuEdgeInset.right, _menuViewHeight) arrowSize:_arrowSize AnchorPoint:CGPointZero];
    ...
    if (self.AnchorPoint.x < BBMenuScreenWidth/2) {
        ...
        if ((self.AnchorPoint.x + menuRect.size.width/2)>(BBMenuScreenWidth-self.menuEdgeInset.right)) {
            menuRect.origin.x = (BBMenuScreenWidth - self.menuEdgeInset.right-menuRect.size.width);
        } else {
            menuRect.origin.x =(self.AnchorPoint.x - menuRect.size.width/2);
        }
    }
    ...
    switch (self.CorrectDirection) {
        case BBMenuControllerArrowUp:
        {
            menuRect.origin.y = self.AnchorPoint.y-menuRect.size.height;
        }
            break;
        case BBMenuControllerArrowDown:
        {
    ...
    if (!CGRectEqualToRect(self.frame, menuRect)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:BBMenuControllerMenuFrameDidChangeNotification object:nil];
    }
    self.frame = menuRect;
...
{
    CGPoint centerPoint = BBMenuGetXCenter(targetRect);
    if (centerPoint.x < 0 || centerPoint.x > BBMenuScreenWidth) {
        centerPoint.x = BBMenuScreenWidth/2;
    }
    if (centerPoint.y < 0 || centerPoint.y > BBMenuScreenHeight) {
        centerPoint.y = BBMenuScreenHeight/2;
    }
    CGPoint targetPoint = CGPointMake(centerPoint.x, centerPoint.y);

    CGSize tempSize = size;
    switch (self.arrowDirection) {
        case BBMenuControllerArrowDefault:
            if ((targetPoint.y - self.arrowMargin - tempSize.height) > BBMenuStatusBarHeight) {
                self.CorrectDirection = BBMenuControllerArrowUp;
                targetPoint.y -= self.arrowMargin;
            } else {
                self.CorrectDirection = BBMenuControllerArrowDown;
        ...
        case BBMenuControllerArrowUp:
            self.CorrectDirection = BBMenuControllerArrowUp;
            if ((targetPoint.y - self.arrowMargin - tempSize.height) < BBMenuStatusBarHeight) {
                targetPoint.y += (BBMenuStatusBarHeight - (targetPoint.y - self.arrowMargin - tempSize.height));
            } else {
                targetPoint.y  -= self.arrowMargin;
            }
            break;
